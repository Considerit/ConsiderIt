namespace :notifications do
  ...
  task :send_out_base_tweets => :environment do
    Account.where(:tweet_notifications => true).each do |account|
      account.proposals.order(:created_at).each do |prop|
        if prop.opinions.published.count > 0
  ...
  task :migrate_to_follow_messaging_system => :environment do
    ...
    Proposal.find_each do |o|
      if o.user && o.opinions.published.where(:user_id => o.user.id).count > 0 && o.opinions.published.where(:user_id => o.user.id).first.notification_point_subscriber
        o.follow!(o.user, :follow => true, :explicit => false)
      end
    end

    #for each opinion, subscribe author to proposal if notification_point_subscriber is true
    Opinion.published.where(:notification_point_subscriber => true ).each do |o|
