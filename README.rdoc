...
6. Install gems.
   a. The mysql gem is problematic on the Macintosh.  Here are quick
   ...
   b. Install the other gems.  For the other gems, note that the Gemfile in the ConsiderIt directory
      specifies a particular version of rails -- this will install that version.
...
Step 1 suggests managing ruby installs with RVM (Ruby Version
...
Step 3 (clone from the git repository) This is appropriate if you just
...
Step 4 (install ImageMagick) For the Macintosh the recommended
technique is to use MacPorts (http://www.macports.org/).  This
involves building from source -- no dmg file, unfortunately.  However,
this has worked fine on various Macs, so you probably won't have any
trouble.

Step 6 (install gems) As noted, the mysql gem is problematic on the
...
(Step 7) Update config/database.yml as necessary
...
and click the "About your application’s environment" link as part of
