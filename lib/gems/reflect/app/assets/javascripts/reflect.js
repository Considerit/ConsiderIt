...
Reflect = {
  ...
  */    
  init : function () {
    ...
    $.plugin( 'bullet', Reflect.entities.Bullet );
    $.plugin( 'comment', Reflect.entities.Comment );
    $.plugin( 'response', Reflect.entities.Response );
    //////////
  ...
  per_request : function() {
    ...
    for (i = 0; i < Reflect.contract.components.length; i += 1) {
      ...
      $( component.comment_identifier + ':not(.rf_comment)' ).each( function () {
    ...
    if ( $('#reflect_templates_present').length > 0 ) {
  ...
  */
  enforce_contract : function () {
    ...
    var user = typeof Reflect.contract.user_name_selector === 'function'
      ...
      : $(Reflect.contract.user_name_selector).text(), i;

    if ( !user || user === '' || user == 'undefined' ) {
      user = Reflect.api.server.get_current_user();
    }

    user_id = $('#m-user-nav .settings').attr('user');

    if ( $('.reflected').length == 0 ) {
      $( Reflect.contract.get_comment_thread() )
          .addClass( 'reflected' );
      $( '.reflected' ).append( '<span id="rf_user_name">' + user + '</span>' );
      $( '.reflected' ).append( '<span id="rf_user_id" class="hide">' + user_id + '</span>' );
    }
    
    for (i = 0; i < Reflect.contract.components.length; i += 1) {
      var component = Reflect.contract.components[i];

      $( component.comment_identifier + ':not(.rf_comment)' )
        .each( function ( index ) {
          $( this ).comment( {
            initializer : component
          });
          var comment = $.data( this, 'comment' );
          if ( Reflect.data && Reflect.data[comment.id]) {
            var bullets = [];
            $.each( Reflect.data[comment.id], function(key, val){
              bullets.push( val );      
            });
            ...
            $.each(bullets, function(key, bullet_info) {
  ...
  */      
  templates : {
    init : function ( templates_from_server ) {      
      if ( templates_from_server ) $( 'body' ).append( templates_from_server );
      
      $.extend( Reflect.templates, {
        bullet : $.jqotec( '#reflect_template_bullet' ),
        new_bullet_prompt : $.jqotec( '#reflect_template_new_bullet_prompt' ),
        new_bullet_dialog : $.jqotec( '#reflect_template_new_bullet_dialog' ),
        bullet_highlight : $.jqotec( '#reflect_template_bullet_highlight' ),
        response : $.jqotec( '#reflect_template_response' ),
        response_dialog : $.jqotec( '#reflect_template_response_prompt' ),
        bullet_rating : $.jqotec( '#reflect_template_bullet_rating'),
        bullet_badge_gallery : $.jqotec( '#reflect_template_ratings_gallery')
      } );      
    }
  },
  ...
  api : {
    ...
    */    
    ...
    post_bullet : function ( event ) {
      ...
      var bullet_obj = $.data( $( event.target )
          .parents( '.bullet' )[0], 'bullet' ), 
        text = $.trim(bullet_obj.elements.bullet_text.html()), 
        highlights = [],
      ...
      bullet_obj.comment.elements.comment_text.find( '.highlight' )
        .each( function(){ 
          highlights.push( $( this ).attr( 'id' ).substring( 9 ) );});
  ...
  handle : {
    ...
    */
    initialize_delegators : function() {
      ...
      //comments
      $('.rf_comment.highlight_state .rf_comment_text .sentence')
        .live('click', Reflect.handle.sentence_click);

      $('.rf_comment.highlight_state .rf_comment_text a.exclude_from_reflect')
        .live('click', function( event ){ event.preventDefault(); });

      $('.rf_comment .rf_toggle_paginate a')
        .live('click', function ( event ) {
          $( event.target ).parents('.summary').find('.bullet_list').children().fadeIn();
      ...
      $('.bullet.full_bullet .bullet_meta .delete')
        .live('click',  function( event ) { $( this ).siblings( '.verification' ).show(); });

      $('.bullet.full_bullet .bullet_meta .delete_nope')
        .live('click',  function( event ) { $( this ).parents( '.verification' ).hide(); });

      $('.bullet.full_bullet .bullet_meta .delete_for_sure')
        .live('click',  function( event ) {
          var bullet_obj = $.data( $( event.target )
              .parents( '.bullet' )[0], 'bullet' );
      ...
      $('.bullet.full_bullet .bullet_meta .modify:enabled')
        .live('click',  function(event) {
          var bullet_obj = $.data( $( event.target )
              .parents( '.bullet' )[0], 'bullet' );
      ...
      $('.bullet.new_bullet .add_bullet:enabled')
        .live('click', function(event) {
          var bullet_obj = $.data( $( event.target )
              .parents( '.bullet' )[0], 'bullet' );
          bullet_obj.enter_edit_state();
      ...
      $('.bullet.modify .bullet_submit:enabled')
        .live('click', function(event) { 
          var bullet_obj = $.data( $( event.target ).parents( '.bullet' )[0], 'bullet' );
      ...
      $('.bullet.modify .cancel_bullet')
        .live('click', function(event) { 
          var bullet_obj = $.data( $( event.target ).parents( '.bullet' )[0], 'bullet' );
          bullet_obj.exit_edit_state( true );
        });

      $('.bullet.connect .submit .bullet_submit:enabled')
        .live('click', Reflect.api.post_bullet);

      $('.bullet.connect .submit .cancel_bullet')
        .live('click', function(event) { 
          var bullet_obj = $.data( $( event.target ).parents( '.bullet' )[0], 'bullet' );
          bullet_obj.exit_highlight_state( true );
          bullet_obj.comment.add_bullet_prompt(); });

      $('.rate_bullet.not_anon .flag')
        .live( 'click', function(event) { Reflect.handle.bullet_flag(event); });

      // responses
      $('.bullet.full_bullet .response_prompt')
        .live('click', function( event ) { 
          $( this ).find('.response_eval').slideDown(); 
          $( this ).find('.action_call').hide();
        });

      $('.bullet.full_bullet .response_prompt .response_maybe')
        .live('click', function( event ) {
          var clarification = $( this ).siblings('.new_response_text');
          if ( clarification.val().length == 0 ) {
            $(this).parents('.response_prompt').find('.bullet_submit').attr('disabled', true);
          }
          clarification
            .focus();
        });

      $('.bullet.full_bullet .response_prompt .response_field')
        .live('click', function( event ) {
          $(this).parents('.response_prompt').find('.bullet_submit').attr('disabled', false);
        });

      $('.bullet.full_bullet .response_prompt .bullet_submit')
        .live('click', function( event ) { 
          var response_obj = $.data( $( event.target )
      ...
      $('.bullet.full_bullet .response_prompt .cancel_bullet')
        .live('click', function( event ) {  
          $(event.target).parents('.response_eval').slideUp();
          $(event.target).parents('.response_prompt').find('.action_call').fadeIn();
          return false;
        } );

      $('.bullet.full_bullet .response_prompt .new_response_text')
        .live('focus', function(){
          $(this).siblings('.response_maybe').attr('checked', true);
        });

      $('.bullet.full_bullet .response_prompt .response_maybe')
        .live('click', function(){
          $(this).siblings('.new_response_text').focus();
        }); 

    },
    bullet_mouseover : function ( event ) {
      var bullet_obj = $.data( $( this )[0], 'bullet' );
    ...
    bullet_mouseout : function ( event ) {      
      var comment = $
        .data( $( event.target ).parents( '.rf_comment' )[0], 'comment' );
    ...
    bullet_flag : function ( event ) {
      var flag_el = $( event.target ).hasClass('flag') ? $( event.target ) : $( event.target ).parents('.flag');
      var flag = flag_el.attr('name'),
          bullet_obj = $.data( $( '#bullet-'+flag_el.parents('.rate_bullet')
    ...
    sentence_click : function ( event ) {
      var parent = $( event.target ).parents( '.rf_comment' ), 
  ...
  * e.g. $('#comment').comment(). Plugin registration accomplished in 
  * Reflect.init.
  */    
  entities : {
    ...
    Comment : {
      ...
      _build : function () {
        ...
        var wrapper = $( '<td id="rf_comment_text_wrapper-'
            + this.id + '" class="rf_comment_text_wrapper">'
            + '<div class=rf_comment_text />' + '</td>' );

        var summary_block = $( '<td id="rf_comment_summary-'
        ...
        var author_block = $( '<span class="rf_comment_author">' 
            + this.user + '</span>' );
        ...
        comment_text
          ...
          .wrapInner( $( '<tr/>' ) )        
          .wrapInner( $( '<table id="rf_comment_wrapper-' 
      ...
      _add_bullet : function ( params ) {
        var bullet = $( '<li />' ).bullet( params );

        this.elements.bullet_list.append( bullet );

        var bullet_obj = $.data( bullet[0], 'bullet' );
    ...
    Bullet : {
      init : function ( options, elem ) {
        this.options = $.extend( {}, this.options, options.bullet_info );
      ...
      _build : function () {
        ...
        this.$elem
          .addClass( 'bullet' )
          .html( $.jqote( Reflect.templates.bullet, template_vars ) );
      ...
      _build_prompt : function () {
        ...
        this.$elem
            ...
            .html( $.jqote( template, template_vars ) )            
      ...
      enter_edit_state : function () {
        var text = this.id ? $.trim(this.elements.bullet_text.html()) : '',
         template_vars = {
        ...
        this.$elem
          .addClass( 'modify' )
          .html( 
            $.jqote( Reflect.templates.new_bullet_dialog, template_vars ) );
      ...
      enter_highlight_state : function () {
        ...
        var child = $('<div />')
            .addClass('rf_dialog')
            .append( $.jqote( Reflect.templates.bullet_highlight ));
      ...
      _add_response : function ( params ) {
        //var response = $( '<li />' ).response( params );
        //this.elements.bullet_eval.find('.rf_rating').after( response );
        var response = $( '<span />' ).response( params );
      ...
      update_badge_gallery : function() {
        ...
        this.$elem.find('.badges')
          .prepend($.jqote( Reflect.templates.bullet_badge_gallery, template_vars ));
        var me = this,
          qtip_settings = $.extend( true, Reflect.default_third_party_settings.qtip(35), {
        ...
        qtip_settings = $.extend( true, Reflect.default_third_party_settings.qtip(50), {
          content: $.jqote( Reflect.templates.bullet_rating, template_vars ),
    ...
    Response : {
      init : function ( options, elem ) {
        this.options = $.extend( {}, this.options, options.response_info );
        this.$elem = $( elem );
      ...
      _build : function () {
        ...
        if ( tag === '2' || tag === '0' ) {
          ...
          var qtip_settings = $.extend(true, Reflect.default_third_party_settings.qtip(140), {
            content : tip,
            style : { width : 140 },
          ...
          this.$elem
            .html($.jqote( Reflect.templates.response, {
                sig : tag,
      ...
      _build_prompt : function () {
        ...
        this.bullet.elements.bullet_main.append($.jqote( Reflect.templates.response_dialog, template_vars ));
        this.$elem.addClass( 'rf_response');
  ...
  utils : {
    ...
    get_logged_in_user : function () {
      if ( typeof Reflect.current_user == 'undefined' ) {
        Reflect.current_user = $( '#rf_user_name' ).text();
      }
    ...
    get_logged_in_user_id : function () {
      if ( typeof Reflect.current_user_id == 'undefined' ) {
        Reflect.current_user_id = $( '#rf_user_id' ).text();
      }
