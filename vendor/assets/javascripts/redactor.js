...
(function($){
	...
	Redactor.prototype = {
		...
		init: function()
		{
			...
			this.setFocusEvents();
			this.$editor.parent().addClass($.trim(this.$editor.text()).length == 0 ? 'empty' : 'text');
		...
		setFocusEvents: function(){
			...
			this.$editor.focus(function(e){
				me.$box.addClass('focused');
				me.$editor.parent().addClass('focused');
			});
			this.$editor.blur(function(e){
				me.$box.removeClass('focused');
				me.$editor.parent().removeClass('focused');
			});		
			this.$toolbar.mousedown(function(e){
				me.$box.addClass('toolbar_focused');
				me.$editor.parent().addClass('toolbar_focused');
			});
			this.$toolbar.mouseup(function(e){
				if (!$(e.target).parent().hasClass('redactor_btn_link') && !$(e.target).parent().hasClass('redactor_btn_formatting'))	{
					me.$box.removeClass('toolbar_focused');
					me.$editor.parent().removeClass('toolbar_focused');
				}
			});
		...
		shortcuts: function()
		{
			$(this.doc).keydown($.proxy(function(e)
			{
				...
				if(!this.$editor.is('.text') && key != 8){
					this.$editor.parent().addClass('text').removeClass('empty');
				} else if($.trim(this.$editor.text()).length == 1 && key == 8) {
					this.$editor.parent().addClass('empty').removeClass('text');				
				}
		...
		{
			var inline_label = this.opts.inline_label ? "<div contenteditable=\"false\" class='label'>" + this.opts.inline_label + "</div>" : '',
				frameHtml = '<!DOCTYPE html>\n' +
				'<html><head><link media="all" type="text/css" href="' + this.opts.path + this.opts.css + '" rel="stylesheet"></head>' +
				'<body><div id="wrapper"><div id="page" contenteditable="true" dir="' + this.opts.direction + '">' + html + 
				'</div>' + inline_label + '</div></body></html>';
