...
$j.fn.infiniteCarousel = function(o) {
    ...
    return this.each(function () {
          ...
          */
          ...
          $(carousel).delegate('.arrow form', 'ajax:success', function (event, response, xhr) {
            ...
            point_col.find('.point_list').append(
              $j($j(response['points']).find('.point_list')[0]).html()
            );
        ...
        function itemHeight() {
            ...
            console.log(page_height + (data.last-data.first) * 10);
            console.log(data.$items.slice(data.first - 1, data.last))
        ...
        function gotoPage(page) {
          ...
          if ( data.resetSizePerPage ) {
            ...
            //data.$wrapper
              //.css('overflow', totalItems() <= data.items_per_page && totalItems() > 0 ? 'visible' : 'hidden');
          }

          data.$items.slice(data.first - 1, data.last).css('visibility', 'visible'); 
          data.$wrapper
            .filter(':not(:animated)').animate(properties, data.speed, function () {
              // set visibility
              //data.$items.slice(0, data.first - 1).css('visibility', 'hidden'); 
              //data.$items.slice(data.last).css('visibility', 'hidden');

              $j(this).parents('.infiniteCarousel').find('.back, .forward').removeAttr('disabled'); 
              //data.$wrapper
              //  .css('overflow', data.vertical ? 'visible' : 'hidden');
