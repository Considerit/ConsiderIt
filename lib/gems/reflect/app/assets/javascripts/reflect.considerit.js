...
(function($) {
  ...
  reflect_server_settings = {
    ...
    reflectPath: subdirectory + '/reflect',
  ...
  $.extend(Reflect.config.api, {
    media_dir: reflect_server_settings.media_dir //these settings need to be stored in Reflect.config.api... - Travis
  });
  
  $.extend(Reflect.config.contract, {
    components: [{
      comment_identifier: '.comment',
      comment_offset:8,
      comment_text:'.comment_body',
      get_commenter_name: function(comment_id){return $.trim( $.trim($('#comment-'+comment_id+ ' .username:first').text()).substring(2));}
    }]
  });
  ...
  Reflect.api.DataInterface = Reflect.api.DataInterface.extend({
    ...
    get_templates: function(callback) {
      $.get(reflect_server_settings.servicesPath + '/get_templates', callback);
    },
    get_current_user: function() {
      var user = $.trim($('#m-user-nav .triangle .settings:visible').text());
      if(!user || user == '')
        user = 'Anonymous';
      return user;
    },
    ...
    post_response: function(settings){
      if (settings.params.response_id) {
        ...
        $.ajax({url:reflect_server_settings.servicesPath + loc,
              type:'POST',
              data: settings.params,
              error: function(data){
                  var json_data = JSON.parse(data);
    ...
    get_data: function(params, callback){
        $.getJSON(reflect_server_settings.servicesPath + '/data', params, callback);
    }
