...
Placer = (opts, bodies) -> 
  ...
  pixelated_layout = ->
    ...
    move_within = Math.round width # * .5 #.125

    # order strategically, placing bigger bodies first, then ordered from the poles inward
    opinions.sort (a,b) ->
      if Math.abs(b.stance - a.stance) < .02 && weights[b.user] != weights[a.user]
    ...
    place_body = (o,idx) ->
      ...
      radius = Math.round Math.sqrt(weight) * base_radius  # circle area of avatar grows linearly with weight 

      adjusted_stance = (o.stance + 1) / 2
      x_target = adjusted_stance * (width - 2 * radius) + radius
      ...
      for consider_only_prime_positions in [true, false]
        ...
        while x <= Math.min(width - radius, xt + move_within) 
          if options[x] > 0
            # score = Math.log(options[col] + 1) / Math.abs( (x_target - col) / width )
            score = 1 / Math.abs(x_target - x)
        ...
        if top_candidate != null
          ...
          if layout_params.jostle
            ...
            if total_dist > 1
              sag = base_radius * layout_params.jostle * Math.random() + .5
        ...
        if !top_candidate && consider_only_prime_positions == false 
    ...
    if !layout_params.show_histogram_initial_layout
      ...
      find_prime = ->
        ...
        while i < width 
          ...
          if y_next?
            ...
            radius_next = Math.dist(hypo_pos, {x: i+dist, y: y_next}) - r_long   # If this point is tracing the side of a circular body, 
                                                                                 # it will be less than or equal to r_long long (the 
          if y_prev?
            disjoint_prev = hypo_pos.y < y_prev 
            radius_prev = Math.dist(hypo_pos, {x: i-dist, y: y_prev}) - r_long
...
EvaluateLayout = (opts, bodies, placer) -> 
  ...
  adj_for_target = layout_params.adj_for_target
  calc_stability = (body, blacklist = {}) -> 
    ...
    if adj_for_target
      adj_angle = Math.atan(Math.abs(body.x - body.x_target) / (height - body.radius - body.y))
      if body.x_target < body.x 
        adj_angle *= -1
      neighbors = []
