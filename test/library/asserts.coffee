...
_.extend casper.test, 
  ...
  assertOnHomepage : ->
    @assertExists '[role="proposal"][state="summary"]', "there is at least one proposal, and it is collapsed"
    @assertElementCount '#active_proposals_region [role="proposal"]', 5, "there are 5 active proposals"
    @assertElementCount '#past_proposals_region [role="proposal"]', 0, "there are no inactive proposals"
  ...
  assertInResultsState : ->
    @assertExists '[role="proposal"][state="results"]', 'Proposal is in results state'
    @assertElementCount '[role="proposal"]', 1, "there is only one proposal on the page"
    ...
    @assertExists '.points_by_community[state="results"]', 'Pros and cons in together state'
  ...
  assertInCraftingState : (state_suffix = '') ->
    @assertExists '[role="proposal"][state="crafting"]', 'Proposal is in crafting state' + state_suffix
    @assertElementCount '[role="proposal"]', 1, "there is only one proposal on the page" + state_suffix
    @assertVisible '.proposal_details', 'Proposal details are visible' + state_suffix
    @assertExists '.decision_board_layout[state="crafting"]', 'Decision slate is visible' + state_suffix
