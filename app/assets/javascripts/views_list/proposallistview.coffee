...
#   #     isAdmin : ConsiderIt.request('user:current').isAdmin()
#   #     selected_sort : @sort_selected
#   #     selected_filter : @filter_selected
#   #     })

#     # $cur_heading = @$el.find('.m-proposals-filters')
...
#     # can_create = ConsiderIt.request('user:current:logged_in?') && (user.isAdmin() || user.isManager() || ConsiderIt.current_tenant.get('enable_user_conversations'))
...
#   #     prompt: if @is_active then "Show more ongoing conversations" else "Show past conversations"
#   #   })
