...
$.fn.dynamicList = function(o) {
    ...
    return this.each(function () {
        ...
        function init(carousel) {
          ...
          $(document).on('click', '#' + data.$carousel.attr('id') + ' .back', function () {
            $(this).parents('.dynamicList').find('.back, .forward').attr('disabled', true);
            return gotoPage(data.currentPage - 1);                
          });

          $(document).on('click', '#' + data.$carousel.attr('id') + ' .forward.not_ajax', function () {
            $(this).parents('.dynamicList').find('.back, .forward').attr('disabled', true);
            return gotoPage(data.currentPage + 1);
          });

          $(document).on('ajax:success', '#' + data.$carousel.attr('id') + ' .arrow form', function (event, response, xhr) {
            var point_col = $(this).parents('.points_other, .full_column'),
                points = $($($(response['points']).find('.point_list')[0]).html());
        ...
        //       page_height += $(this).outerHeight(true);
        ...
        function gotoPage(page) {
          ...
          var items_to_hide = [];
          $.merge(items_to_hide, data.$items.slice(0, data.first - 1));
          $.merge(items_to_hide, data.$items.slice(data.last));
          items_to_hide = $(items_to_hide).filter(':visible');

          if (items_to_hide.length > 0) {
            $(items_to_hide).filter(':visible').fadeOut(function(){
