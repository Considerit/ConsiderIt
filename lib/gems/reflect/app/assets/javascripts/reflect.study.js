...
Reflect.study = {
			...
			checkboxes, survey_id, element ) 
	{
		...
		function open () {
			$( this ).parents( '.survey_prompt' ).find( '.survey_detail' )
					.slideDown();
			$( this ).unbind( 'click' );
			$( this ).click( close );
		}
	...
	post_bullet_mousehover : function ( event ) {
		try {
		  	var id = $(event.target).parents('.bullet').attr('id').substring(7);
	...
	post_comment_mousehover : function ( event ) {
		var id = $(event.target).parents('.comment').attr('id').substring(8);
		Reflect.study.post_mousehover(1, id);	
	},	
	post_blog_mousehover : function ( event ) {
		var id = $(event.target).parents('.blogpost_body').attr('id').substring(9);
	...
	instrument_bullet : function( element ) {
		...
		$(element).find('.bullet_main_wrapper').hoverIntent( config );
	...
	instrument_mousehovers : function (  ) {
		...
		$(".bullet_main_wrapper").hoverIntent( config );
		
		config.over = Reflect.study.post_comment_mousehover;
		$(".rf_comment_text_wrapper").hoverIntent( config );
		
		config.over = Reflect.study.post_blog_mousehover;
		$(".blogpost_body").hoverIntent( config );		
	}
};
